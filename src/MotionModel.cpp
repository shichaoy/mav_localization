...
namespace MAV_localization
{
   ...
   MotionModel::MotionModel ( ros::NodeHandle* nh, EngineT* rngEngine, tf::TransformListener* tf,
      ...
      m_tfListener ( tf ), m_rngNormal ( *rngEngine, NormalDistributionT ( 0.0, 1.0 ) ), m_rngUniform ( *rngEngine,
            UniformDistributionT ( 0.0, 1.0 ) ), m_odomFrameId ( odomFrameId ), m_baseFrameId ( baseFrameId ), m_firstOdometryReceived (
               false )
   ...
   tf::Transform MotionModel::odomTransformNoise ( const tf::Transform& odomTransform )
   {
      ...
      return tf::Transform ( tf::createQuaternionFromRPY (
                             ...
                             tf::Vector3 ( m_rngNormal() * sqrt ( motion_variance ( 0 ) ), // x
   ...
   void MotionModel::applyOdomTransform ( Particles& particles, const tf::Transform& odomTransform )
   {
      ...
      tf::Transform tf_noise;
      tf_noise = odomTransformNoise ( odomTransform );
      ROS_DEBUG_STREAM ( "odometry noise is: "
                         << tf_noise.getOrigin().getX() << "   "
                         << tf_noise.getOrigin().getY() << "   "
                         << tf_noise.getOrigin().getZ() );
   ...
   void MotionModel::addRandomNoise ( Particles& particles )
   {
      for ( unsigned i = 0; i < particles.size(); ++i )
      {
         tf::Transform tf_noise;
         tf_noise = tf::Transform ( tf::createQuaternionFromRPY (
                                       m_rngNormal() * 0.01,     // roll
                                       m_rngNormal() * 0.01,     // pitch
                                       m_rngNormal() * 0.05 ),   // yaw
                                    tf::Vector3 (
                                       ( m_rngNormal() + 0.2 ) * 0.05,   // x
                                       m_rngNormal() * 0.01,      // y
                                       m_rngNormal() * 0.01 ) );  // z;
         ROS_DEBUG_STREAM ( "odometry noise is: "
                            << tf_noise.getOrigin().getX() << "   "
...
//     ros::WallTime startTime = ros::WallTime::now();
      ...
      {
         ROS_WARN ( "Failed to apply temporal sampling uncertainty for motion model!" );
         return false;
      }
...
//     ROS_INFO_STREAM ( "OdomTransformTemporal took " << dwalltime << "s " );
   ...
   bool MotionModel::lookupOdomPose ( const ros::Time& t, tf::Stamped<tf::Pose>& odomPose ) const
   {
      ...
      try
      {
	// Since VO sometimes is a little late, so we need to wait sometime.
         m_tfListener->waitForTransform ( m_odomFrameId, m_baseFrameId, t, ros::Duration ( 0.5 ) );
