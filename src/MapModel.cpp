...
namespace MAV_localization
{
   MapModel::MapModel ( ros::NodeHandle* nh ) :
      m_motionMeanZ ( 0.0 ), m_motionRangeZ ( -1.0 ), m_motionRangeRoll ( -1.0 ), m_motionRangePitch ( -1.0 ), m_motionObstacleDist (
         0.2 )
   {
   ...
   void MapModel::verifyPoses ( Particles& particles )
   {
      ...
      for ( unsigned i = 0; i < particles.size(); ++i )
      {
         ...
         octomap::point3d position ( particles[i].pose.getOrigin().getX(), particles[i].pose.getOrigin().getY(),
                                     particles[i].pose.getOrigin().getZ() );

         // see if outside of map bounds:
         if ( position ( 0 ) < minX || position ( 0 ) > maxX || position ( 1 ) < minY || position ( 1 ) > maxY || position ( 2 ) < minZ
               || position ( 2 ) > maxZ )
         {
         ...
         else
         {
            ...
            {
               // see if current pose is has a valid height:
               if ( m_motionRangeZ >= 0.0
                     && ( std::abs (
                             particles[i].pose.getOrigin().getZ() - getFloorHeight ( particles[i].pose )
      ...
      {
         ROS_INFO ( "Particle weights minimized: %d out of map, %d in obstacles, %d out of motion range", numOut, numWall,
                    numMotion );
      }
   ...
   void MapModel::initGlobal ( Particles& particles, double z, double roll, double pitch, const Vector6d& initNoise,
...
//             it->pose.getOrigin().setZ ( z_list.at ( zIdx ) + z + rngNormal() * initNoise ( 2 ) );
            it->pose.getOrigin().setZ ( z + rngNormal() * initNoise ( 2 ) );

            std::cout << "set z: " << z + rngNormal() * initNoise ( 2 )  << std::endl;
...
// 	    std::cout << "Z: " << z_list.at(zIdx) << std::endl;
         }
      }
...
//     }
      ...
      while ( z >= minZ )
      {
         for ( int u = -2; u <= 2; u ++ )
         {
            for ( int v = -2; v <= 2; v ++ )
            {
               if ( isOccupied ( octomap::point3d ( x + u * res, y + v * res, z ) ) )
               {
                  inside_map = true;
               }
            }
...
// Groovy:
...
#else  // Fuerte:
      m_map.reset ( (octomap::OcTree*)octomap_msgs::binaryMsgDataToMap ( resp.map.data ) );
#endif
      ...
      double x, y, z;
      m_map->getMetricSize ( x, y, z );
      ROS_INFO ( "Occupancy map initialized with %zd nodes (%.2f x %.2f x %.2f m), %f m res.", m_map->size(), x, y, z,
                 m_map->getResolution() );
